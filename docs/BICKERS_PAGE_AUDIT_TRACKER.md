...
Change policy: this file is an inventory only. No page or workflow audit has started, and no application code was changed while creating it.
...
| Clerk login | `/login` | Clerk sign-in and authentication feedback. | Public; should redirect authenticated users appropriately. | Clerk UI, `GlobalThemeProvider`, `ContentLabelsProvider`, login layout. | Clerk-hosted authentication; login/security endpoints may be called by utilities. | Clerk session; no direct business collection. | `/auth/complete`, Firebase bridge, setup/verify MFA. | Not reviewed |
| Authentication completion | `/auth/complete` | Resolves the signed-in user’s landing workspace and sends login notification. | Signed-in Clerk user; enabled mapped account intended. | `AuthProvider`, access-control and login-notification utilities. | `/api/security/login-notification`; bootstrap occurs through context. | `users`, `employees`, `loginSecurityLogs`. | Home/service landing, MFA. | Not reviewed |
| Clerk-to-Firebase session bridge | All private routes | Exchanges a verified Clerk session with an explicit UID link for a versioned Firebase token, then validates and mirrors fail-closed canonical access. | Verified Clerk user with an explicit Clerk↔Firebase UID link; enabled and internally consistent canonical/employee/company access. | `AuthProvider`, `ProtectedLayout`, `AccountGuard`. | `/api/auth/firebase-token`, `/api/security/bootstrap-access`. | Firebase Auth; `users`, `employees`, `settings`, `platformCompanies`, `adminAuditLogs`, `loginSecurityLogs`. | Every Firestore-backed page. | Testing completed |
| MFA enrolment | `/setup-mfa` | Enrol authenticator, verify setup, refresh access and security state. | Signed-in mapped user; MFA feature enabled. | auth-security, access-control and login-notification utilities. | `/api/mfa/setup`, `/api/mfa/verify`, `/api/security/bootstrap-access`. | `users`; server-only `mfaSecrets`; `adminAuditLogs`, `loginSecurityLogs`. | `/verify-mfa`, login completion. | Not reviewed |
| MFA challenge | `/verify-mfa` | Verify an existing authenticator challenge and continue to the appropriate workspace. | Signed-in mapped user requiring current MFA verification. | `AuthProvider`, auth-security and access-control utilities. | `/api/mfa/verify`, `/api/security/bootstrap-access`. | `users`; server-only `mfaSecrets`; security logs. | `/setup-mfa`, home/service landing. | Not reviewed |
...
| Request middleware | All matched application and API paths | Runs Clerk middleware. Current inventory found no explicit private-route matcher calling `auth.protect()`. | Must distinguish public auth routes from private pages and APIs. | `src/middleware.js`. | All API handlers are matched but must still authorize themselves. | None directly. | Authentication boundary. | Not reviewed |
...
| Access and tenant helpers | All Firestore business workflows | Maps route prefixes to workspaces/modules and creates collection queries/payloads. | Intended to enforce enabled user, company, workspace and module access. | `accessControl.js`, `firestoreAccess.js`, `storageAccess.js`. | None directly. | All tenant collections and Storage paths. | Firestore/Storage rules; all pages. | Not reviewed |
...
## Authentication audit progress — explicit identity bootstrap

Updated: 21 July 2026

Scope reviewed and changed:

**Verified Clerk session/email → explicit Clerk↔Firebase UID link → versioned Firebase token → explicit employee UID resolution → canonical users/{uid} creation or refresh.**

### Rejected transfer and replacement scope

The previously proposed OneDrive cleanup for `src/app/api/security/bootstrap-access/route.js` was rejected and remains unapplied. It removed only dead email-parameter code and left employee-document-ID fallback, canonical UID overwrite, cross-company mixing and implicit access defaults intact.

A separately approved hardening changed:

- `src/app/api/auth/firebase-token/route.js`
- `src/app/api/security/bootstrap-access/route.js`
- `src/app/api/admin/_lib.js` (essential additive verified-claim propagation)
- `tests/identityBootstrap.test.mjs` (new focused exact-source tests)

### Identity and access invariants now enforced

- Clerk authentication and a verified `@bickers.co.uk` email are required.
- UID selection uses only Clerk `externalId`, Clerk private metadata `firebaseUid`, or a unique canonical `clerkUserId`/`auth.clerkUserId` link. Email is diagnostic/consistency evidence only.
- The Firebase token must carry version-2 verified-Clerk assurance created by the hardened bridge.
- Existing canonical `users/{uid}.uid` must equal the authenticated UID; conflicts are denied and never repaired automatically.
- Ordinary users require exactly one enabled employee whose explicit `authUid`/`uid` equals that UID. Employee document IDs are ignored for identity.
- Duplicate, conflicting, other-UID and changed employee links fail closed.
- Canonical/employee email, employeeId and company conflicts fail closed.
- Missing company fails for non-Platform-Admin users.
- Only canonical `admin` and `platformAdmin` roles retain administrative behavior; email allowlists and legacy elevation aliases are not used.
- Missing workspace configuration returns 403. Explicit `user:false, service:false` remains denied and does not gain a default workspace.
- Missing module configuration produces and persists false flags; explicit module booleans are honored.
- Request bodies cannot choose identity or access fields.
- A missing canonical ordinary-user record can be created only after the versioned token, unique explicit employee link, company, email and workspace checks succeed.
- The canonical document is written atomically; failed writes return 500 before success auditing, and repeated converged requests are idempotent.
- Denials write non-sensitive `loginSecurityLogs` events where logging is available.
...
| Focused identity-boundary tests | Passed, 26/26 with Node's required `--experimental-vm-modules` flag; a plain `node --test` verification attempt failed before assertions because `vm.SyntheticModule` was unavailable |
| Access-control tests | Passed, 12/12 |
| Authentication-boundary tests | Passed, 3/3 |
| Caller/contract review | Passed; successful bridge/bootstrap response shapes remain compatible |
| Reverse-patch validation | Passed for all source/test changes |
| Firestore emulator | Blocked: Firebase CLI 15.24.0 requires Java 21+, environment provides Java 17.0.18 |
| Live Clerk/Firebase browser flow | Not run |
| Production identity/data validation or mutation | Not run |

### Manual remediation and remaining blockers

Before rollout, manually inventory and resolve:

- Clerk accounts without an explicit Firebase UID link.
- Employees without `authUid`/`uid`.
- Duplicate, conflicting or orphaned employee UID links.
- Canonical document-ID/data.uid mismatches and employeeId conflicts.
- Cross-company user/employee links.
- Missing explicit `appAccess` booleans.
- Missing company/platform module configuration.

Exact current production counts are unknown because production and Clerk data were not read. Historical audit figures recorded 12 employees without auth UID and 3 orphaned employee UID links; they were not revalidated. Counts for missing Clerk links and missing workspace/module configuration remain unavailable.

### Status decision

**Testing completed** for this tightly scoped bridge/bootstrap code boundary. The changes are safe to retain and ready for the later authentication consolidation commit as one group.

Operational approval remains **Blocked** until manual identity/access inventory is completed and a real Clerk → Firebase → bootstrap browser flow succeeds. The wider authentication item also remains open for MFA assurance, route protection, Firestore rules and Storage rules; no next audit item was started.

### Commit-readiness validation follow-up — 21 July 2026

The retained boundary required three narrow corrections during complete-flow validation:

- The Firebase bridge now rejects any employee UID-field conflict tied to the resolved UID before issuing a token, regardless of email.
- `AuthProvider` now signs out fail-closed when bootstrap is denied or unavailable and cannot continue from a stale direct `users/{uid}` read.
- Bootstrap audit entries now contain only allowlisted access fields rather than arbitrary canonical-user data.

A gated read-only identity diagnostic was added at `scripts/identity-access-inventory.mjs`. It was tested with fixture data only and was not run against Clerk or production Firebase. After separate approval, its production command is:

`IDENTITY_INVENTORY_CONFIRM_READ_ONLY=1 node scripts/identity-access-inventory.mjs --production --project=bickers-booking`

Final local results: syntax passed; targeted ESLint passed; focused identity tests passed 30/30; access-control tests passed 12/12; authentication-boundary tests passed 3/3; import, formatting and CLI safety checks passed. Firestore emulator execution remains blocked because Java 17.0.18 is installed and Firebase CLI 15.24.0 requires Java 21.

Commit readiness is **READY TO COMMIT WITH MANUAL FOLLOW-UP**. Operational approval remains **Blocked** pending Java 21 emulator testing, approved read-only identity inventory/remediation and a real browser flow. No next audit item was started.
