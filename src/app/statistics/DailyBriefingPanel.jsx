...
import { AlertTriangle, ArrowRight, BrainCircuit, CheckCircle2, ShieldCheck, ThumbsDown, ThumbsUp } from "lucide-react";
...
export default function DailyBriefingPanel() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (!cancelled) setState((current) => ({ ...current, loading: false, error: error.message || "Briefing unavailable." }));
      }
    });
  ...
  const submitFeedback = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/statistics/daily-briefing/feedback", {
        ...
        body: JSON.stringify({ rating: feedback, reason, briefingDate: state.briefing.briefingDate, insightId: "briefing" }),
      });
  ...
  if (!state.briefing) return <section style={shell}><strong>Today&apos;s briefing has not been generated yet.</strong><div style={{ marginTop: 5, color: "var(--legacy-color-5f6f82)" }}>The first approved briefing is prepared at 06:00 UK time.</div></section>;
    ...
    <section style={shell} aria-labelledby="daily-briefing-title">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "grid", gap: 4, justifyItems: "end", color: "var(--legacy-color-5f6f82)", fontSize: 11.5 }}>
          <span>{state.stale ? <AlertTriangle size={13} style={{ verticalAlign: "-2px", marginRight: 4 }} /> : <CheckCircle2 size={13} style={{ verticalAlign: "-2px", marginRight: 4 }} />}{state.stale ? "Previous briefing" : `Generated ${fmtDateTime(briefing.generatedAt)}`}</span>
          ...
          return (
            <article key={insight.id} style={{ border: `1px solid ${colors.border}`, background: colors.background, borderRadius: 9, padding: 11, display: "flex", flexDirection: "column", minHeight: 215 }}>
              ...
              <div style={{ marginTop: "auto", paddingTop: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                ...
                {insight.action?.href ? <Link href={insight.action.href} style={{ display: "inline-flex", alignItems: "center", gap: 4, color: "var(--legacy-color-1f4b7a)", fontSize: 12, fontWeight: 850, textDecoration: "none" }}>{insight.action.label}<ArrowRight size={13} /></Link> : null}
              </div>
            </article>
          );
        })}
