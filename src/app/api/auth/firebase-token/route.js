...
export async function POST() {
  try {
    ...
    if (userMatches.some(({ data }) => data?.credentialResetRequired === true)) {
      return Response.json(
        { error: "You must reset your Clerk password before access can be restored.", code: "credential_reset_required" },
        { status: 403 }
      );
    }
    ...
    const linkedUser = userMatches.find(
      ({ id, data }) => safeUid(data?.uid || id) === uid
    );
    if (!linkedUser || linkedUser.data?.isEnabled !== true) {
      return Response.json({ error: "No active application access record was found." }, { status: 403 });
    }
    const companyId = String(linkedUser.data?.companyId || "").trim();
    if (!companyId) {
      return Response.json({ error: "No valid company membership was found." }, { status: 403 });
    }
    ...
    return Response.json({ customToken, uid, email, companyId });
  } catch (error) {
