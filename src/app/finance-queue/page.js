...
export default function ReadyToInvoiceTablePage() {
  ...
  // Ready to invoice & not paid
  const queue = useMemo(() => {
    return jobs
      .filter((j) => !isPaid(j))
      .filter((j) => /ready\s*to\s*invoice/i.test(String(j.status || "")) || !!j.readyToInvoice)
      .sort((a, b) => {
  ...
  const Table = ({ jobs }) => (
    <div style={tableWrap}>
      <table style={tableEl} aria-label="Ready to Invoice">
        <thead>
          <tr>
            <th style={th}>Job #</th>
            ...
            <tr key={job.id}>
              ...
              <td style={td}>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  {/* Always show Ready to Invoice badge, since this page is that filter */}
                </div>
              </td>
              <td style={td}>
                <Link href={`/job-summary/${job.id}`} style={{ textDecoration: "none", fontWeight: 800, color: UI.brand }}>
                  Invoice →
                </Link>
              </td>
            </tr>
  ...
  return (
    <HeaderSidebarLayout>
      <div style={pageWrap}>
        ...
        <div style={headerBar}>
          <div>
            <h1 style={h1}>Ready to Invoice</h1>
            <div style={sub}>Only jobs flagged as Ready to Invoice (status or flag), not paid. Grouped by week.</div>
          </div>
        ...
        <div style={toolbar}>
          <div style={searchWrap} title="Press / to focus">
            ...
            <input
              ...
              style={searchInput}
              aria-label="Search ready-to-invoice jobs"
            />
          </div>
        ...
        <div style={{ marginTop: 14 }}>
          {loading ? (
            ...
            <div style={{ ...surface, padding: 24, textAlign: "center", color: UI.muted }}>No jobs Ready to Invoice.</div>
          ) : (
            <>
              ...
              {(noDate?.length ?? 0) > 0 && (
                <section style={{ marginTop: 8 }}>
                  <div style={sectionHeader}>
                    ...
                    <span style={tinyHint}>Jobs without booking dates</span>
                  </div>
                  <Table jobs={noDate} />
                </section>
              )}
