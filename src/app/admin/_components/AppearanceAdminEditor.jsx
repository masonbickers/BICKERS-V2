...
export default function AppearanceAdminEditor({ section }) {
  ...
  return <HeaderSidebarLayout><Page width="fluid">
    ...
    <div className={styles.actionBar}><Button variant="secondary" onClick={() => setDraft(defaults)}><RotateCcw size={15} /> Restore defaults</Button><Button variant="secondary" onClick={() => mutate("POST", { action: "discard" }, "discard")} disabled={busy || !changes}>Discard draft</Button><Button variant="secondary" loading={busy === "save"} onClick={() => mutate("PATCH", { draft }, "save")} disabled={!dirty}><Save size={15} /> Save draft</Button><Button loading={busy === "publish"} onClick={() => mutate("POST", { action: "publish" }, "publish")} disabled={changes === 0 || !validation.valid}><Send size={15} /> Publish</Button></div>
