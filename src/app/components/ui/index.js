...
export function Modal({ open, onClose, title, description, children, footer, size = "md", closeLabel = "Close", className = "" }) {
  ...
  if (!open) return null;
  return <div className={styles.modalBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose?.(); }}><section ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="shared-modal-title" className={cx(styles.modal, size === "sm" && styles.modalSm, size === "lg" && styles.modalLg, className)}><header className={styles.modalHeader}><div><h2 id="shared-modal-title" className={styles.modalTitle}>{title}</h2>{description ? <p className={styles.modalDescription}>{description}</p> : null}</div><IconButton label={closeLabel} variant="ghost" onClick={onClose}>×</IconButton></header><div className={styles.modalBody}>{children}</div>{footer ? <footer className={styles.modalFooter}>{footer}</footer> : null}</section></div>;
}
