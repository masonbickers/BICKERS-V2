...
const UI = { text:"var(--legacy-color-111827)", muted:"var(--legacy-color-6b7280)", bg:"var(--color-white)", border:"1px solid var(--legacy-color-e5e7eb)", radiusLg:12, radius:8, shadow:"0 6px 16px rgba(0,0,0,0.06)" };
const card = { background:UI.bg, border:UI.border, borderRadius:UI.radiusLg, boxShadow:UI.shadow, padding:"var(--space-4)" };

const Row = ({ label, value }) =>
  value ? (
    <div style={{ display:"grid", gridTemplateColumns:"160px 1fr", gap:"var(--space-3)", margin:"6px 0" }}>
...
export default function RecceFormPage() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--legacy-color-f3f4f6)", padding:"var(--space-6)" }}>
      <div style={{ maxWidth:900, margin:"0 auto", display:"grid", gap:"var(--space-4)" }}>
        {/* Header */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <h1 style={{ margin:0, fontSize:"var(--font-size-xl)", fontWeight:800, color:UI.text }}>Recce Form</h1>
          <div style={{ display:"flex", gap:"var(--space-2)" }}>
            <button onClick={() => router.back()} style={{ padding:"8px 12px", borderRadius:"var(--radius-md)", border:"1px solid var(--legacy-color-e5e7eb)", background:"var(--legacy-color-f9fafb)", fontWeight:700, cursor:"pointer" }}>← Back</button>
            <button onClick={() => window.location.reload()} style={{ padding:"8px 12px", borderRadius:"var(--radius-md)", border:"1px solid var(--legacy-color-e5e7eb)", background:"var(--legacy-color-f9fafb)", fontWeight:700, cursor:"pointer" }} title="Force refresh">Refresh</button>
            <button onClick={() => window.print()} style={{ padding:"8px 12px", borderRadius:"var(--radius-md)", border:"1px solid var(--legacy-color-111827)", background:"var(--legacy-color-111827)", color:"var(--color-white)", fontWeight:700, cursor:"pointer" }}>Print</button>
          </div>
        </div>
          ...
          {!loading && !error && recce && (
            <div style={{ display:"grid", gap:"var(--space-4)" }}>
              <div style={{ display:"flex", flexWrap:"wrap", gap:"var(--space-2)" }}>
                <span style={{ fontSize:"var(--font-size-xs)", fontWeight:800, padding:"4px 8px", borderRadius:"var(--radius-pill)", background:String(recce.status||"").toLowerCase()==="approved"?"var(--legacy-color-34d399)":"var(--legacy-color-7cff9a)", border:"1px solid var(--legacy-color-0b0b0b)", color:"var(--legacy-color-111)" }}>
                  {(recce.status || "Submitted").toUpperCase()}
                </span>

                {recce.bookingId && (
                  <span title="Booking ID" style={{ fontSize:"var(--font-size-xs)", fontWeight:800, padding:"4px 8px", borderRadius:"var(--radius-pill)", background:"var(--legacy-color-e5e7eb)", border:"1px solid var(--legacy-color-0b0b0b)", color:"var(--legacy-color-111)" }}>
                    Booking: {recce.bookingId}
                  </span>
                )}

                {submittedAt && (
                  <span style={{ fontSize:"var(--font-size-xs)", fontWeight:800, padding:"4px 8px", borderRadius:"var(--radius-pill)", background:"var(--legacy-color-e5e7eb)", border:"1px solid var(--legacy-color-0b0b0b)", color:"var(--legacy-color-111)" }}>
                    Submitted {submittedAt.toLocaleString("en-GB")}
                  </span>
                )}

                {updatedAt && (
                  <span style={{ fontSize:"var(--font-size-xs)", fontWeight:800, padding:"4px 8px", borderRadius:"var(--radius-pill)", background:"var(--legacy-color-eef2ff)", border:"1px solid var(--legacy-color-0b0b0b)", color:"var(--legacy-color-111)" }}>
              ...
              <div>
                ...
                <div style={{ marginTop:18 }}>
                  <div style={{ fontWeight:800, color:UI.text, marginBottom:"var(--space-2)" }}>Photos</div>
                  {photoUrls.length === 0 ? (
                ...
                {Object.keys(answers).length > 0 && (
                  <details style={{ marginTop:"var(--space-3)" }}>
                    ...
                    <pre style={{ marginTop:"var(--space-2)", fontSize:"var(--font-size-xs)", background:"var(--color-surface-subtle)", padding:"var(--space-3)", borderRadius:"var(--radius-md)", overflowX:"auto" }}>
