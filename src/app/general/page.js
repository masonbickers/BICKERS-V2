...
function extractApprovedGeneral(checkDocs) {
  ...
  for (const c of checkDocs) {
    ...
    c.items.forEach((it, idx) => {
      const r = it?.review;
      if (r?.status === "approved" && r?.category === "general") {
        rows.push({
          checkId: c.id,
          defectIndex: idx,
...
export default function GeneralDefectsPage() {
  ...
  const [savingId, setSavingId] = useState(null); // checkId|idx for UI lock
  ...
  useEffect(() => {
    ...
    if (!gate.allowed) {
      reportDataAccessBlocked(gate, { collectionName: "vehicleChecks", operation: "load general defects" });
      setRows([]);
      setLoading(false);
      return;
    }
  ...
  const rerouteToImmediate = async (row) => {
    ...
    try {
      ...
      // take them to Immediate list (nice flow)
                    ...
                    return (
                      <tr key={key}>
                        ...
                        <td style={td}>
                          ...
                          {m === "scheduled" && <span style={badge("var(--color-success-soft)", "var(--legacy-color-065f46)")}>Scheduled</span>}
                        ...
                        <td style={{ ...td, textAlign: "right", whiteSpace: "nowrap" }}>
                          ...
                          <button
                            style={btn("var(--color-danger-soft)", "var(--color-danger)")}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {notesModal.newStatus === "scheduled"
        ...
        <textarea
          ...
          placeholder={
            notesModal.newStatus === "scheduled"
              ? "e.g., Booked in bodyshop for next Tuesday."
              : "e.g., Replaced tyre and cleared warning light."
          }
          style={{
            width: "100%",
            border: "1px solid var(--legacy-color-e5e7eb)",
