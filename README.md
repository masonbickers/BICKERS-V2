...
Legacy Firebase setup-code and application-native passkey login routes and feature flags have been removed. Disable Firebase Email/Password sign-in with `npm run incident:disable-legacy-auth -- --apply` after the incident dry-run is reviewed; Firebase custom-token sign-in remains enabled solely for the Clerk-to-Firebase data-access bridge.
