...
Legacy Firebase setup-code and passkey login token routes return `410` by default. `ALLOW_LEGACY_FIREBASE_LOGIN=true` exists only as a short-term emergency rollback switch and should remain off after Clerk is live.
...
3. During the compatibility phase, confirm the mapped Firebase Auth UID still exists and is enabled.
